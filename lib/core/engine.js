...
class Engine {
  constructor(options) {
    try {
      ...
      if (options.seleniumScript) {
        options.seleniumScript = loadRunScript(options.seleniumScript);
      }
    } catch (e) {
      log.error(e.message);
      throw e;
      ...
      .tap(result => {
        ...
        if (result.har) {
          ...
          for (let run = 0; run < numPages; run++) {
            ...
            addExtrasToHAR(page, visualMetric, browserScript.timings, options);
          }
        }
      })
      .tap(result => {
