...
class Statistics {
  ...
  summarize(options) {
    ...
    return Object.keys(this.data).reduce((results, key) => {
      ...
      if (options.statisticsAccuracy) {
        results[key].moe = Number(stats.moe().toFixed(2));
        results[key].sd = Number(stats.σ().toFixed(2));
        results[key].gsd = Number(stats.gstddev().toFixed(2));
      }
  ...
  summarizeDeep(options) {
    ...
    function summarize(stats) {
      ...
      if (options.statisticsAccuracy) {
        results.moe = Number(stats.moe().toFixed(2));
        results.sd = Number(stats.σ().toFixed(2));
        results.gsd = Number(stats.gstddev().toFixed(2));
      }
